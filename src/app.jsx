...
function App() {
  ...
  const [aiMode, setAiMode] = React.useState("sidebar");
  const [searchOpen, setSearchOpen] = React.useState(false);
  const [patientCardOpen, setPatientCardOpen] = React.useState(true);
  const [apptSide, setApptSide] = React.useState(null);
  const onToggleAside = () => setAiMode(m => m === "sidebar" ? "closed" : "sidebar");

  React.useEffect(() => {
    const onOpen = () => setSearchOpen(true);
    const onTogglePatient = () => {
      setPatientCardOpen(open => {
        const next = !open;
        if (next && aiMode === "closed") setAiMode("sidebar");
        return next;
      });
    };
    const onKey = (e) => {
    ...
    window.addEventListener("toggle-patient-card", onTogglePatient);
    window.addEventListener("keydown", onKey);
    return () => {
      window.removeEventListener("open-search", onOpen);
      window.removeEventListener("toggle-patient-card", onTogglePatient);
  ...
  const patientCardData = (() => {
    if (page === "patient-detail" && openedPatient) {
      const p = openedPatient;
      return {
        patient: {
          name: p.name, species: p.species, breed: p.breed,
          sex: p.sex === "F" ? (p.neutered ? "F (sp.)" : "F") : (p.neutered ? "M (n.)" : "M"),
          age: p.age, weight: p.weight, microchip: "956 000 010 234 567",
          lastVisit: p.lastVisit || "3 days ago", nextAppt: "Apr 30 · 10:30",
        },
        client: {
          name: p.owner || "Sarah Andersen", since: "Jan 2022",
          phone: "+44 7700 900 305",
          email: (p.owner || "sarah.andersen").toLowerCase().replace(/\s+/g, ".") + "@gmail.com",
          balance: "£0.00",
        },
      };
    }
    if (page === "client-detail" && openedClient) {
      const c = openedClient;
      const firstPet = (c.pets && c.pets[0]) || null;
      return {
        patient: firstPet ? {
          name: firstPet.name, species: firstPet.species, breed: firstPet.breed,
          sex: firstPet.sex, age: firstPet.age, weight: firstPet.weight,
          microchip: "956 000 010 234 567", lastVisit: firstPet.lastVisit || "—",
          nextAppt: "Apr 30 · 10:30",
        } : null,
        client: {
          name: c.name, since: c.since || "Jan 2022",
          phone: c.phone || "+44 7700 900 305", email: c.email,
  ...
  const showPatientCard = !!patientCardData && patientCardOpen && showAside;
      ...
      <div className={"main" + (showAside ? " main--with-aside" : "")}>
        ...
        {showAside && (
          <div className="right-rail">
            {showPatientCard ? (
              <window.PatientSummaryCard
                patient={patientCardData.patient}
                client={patientCardData.client}
                chatProps={{ mode: "sidebar", setMode: setAiMode, onClose: closeAi }}
