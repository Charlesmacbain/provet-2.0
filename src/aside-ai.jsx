...
const LAST_OPEN_MODE_KEY = "provet.askAi.lastOpenMode";
function readLastOpenMode() {
  try {
    const v = localStorage.getItem(LAST_OPEN_MODE_KEY);
    return v === "sidebar" || v === "floating" ? v : "floating";
  } catch { return "floating"; }
}
function writeLastOpenMode(m) {
  if (m !== "sidebar" && m !== "floating") return;
  try { localStorage.setItem(LAST_OPEN_MODE_KEY, m); } catch {}
}

function AskAI({ mode = "closed", setMode: setModeProp, onClose }) {
  ...
  if (mode === "closed" || (mode === "floating" && !showChat)) {
    return <AskPill onClick={() => {
      if (mode === "closed") {
        const last = readLastOpenMode();
        setMode(last);
        if (last === "floating") setShowChat(true);
      } else {
        setShowChat(true);
      }
    }} />;
  ...
  const { useDragResize: useDragResizeAI, ResizeHandle: ResizeHandleAI } = window.PROVET_PRIM;
  const [aiWidth, onAiResize] = useDragResizeAI({ initial: 320, min: 260, max: 640, side: "left", storageKey: "provet.askAi.width" });
  return (
    <aside className="aside aside--ai" style={{ width: aiWidth, position: "relative" }}>
      <ResizeHandleAI side="left" onMouseDown={onAiResize} />
      <ChatPanel mode={mode} setMode={setMode}
        onClose={onClose} />
    </aside>
  );
